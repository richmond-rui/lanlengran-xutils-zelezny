...
public class InjectWriter extends WriteCommandAction.Simple {
    ...
    private void generateMultipleClickMethods() {
        for (Element element : mElements) {
            if (element.isClick) {
                StringBuilder method = new StringBuilder();
                method.append("@Event(" + element.getFullID() + ")");
...
//            method.append("android.view.View rootView = super.onCreateView(inflater, container, savedInstanceState);\n");
            ...
            method.append("return "+butterKnife.getCanonicalBindStatement()+"(this, inflater,container);\n");
            method.append("}");
    ...
    private void addBindHeader() {
        try{
            String realName=mLayoutFileName.split(".")[0];
            String bindHeader="@ContentView(R.layout."+realName+")";

            boolean isHasHeader=mClass.getText().contains(bindHeader);
            if (!isHasHeader){
                mClass.addBefore(mFactory.createStatementFromText(bindHeader, mClass),mClass);
            }
        }catch (Exception e){
            e.printStackTrace();
        }
    ...
    private String getNameForUnbinder(@NotNull IButterKnife butterKnife) {
        ...
        String unbinderName = "rootView";
        int idx = 1;
        while (mClass.findFieldByName(unbinderName, false) != null) {
            unbinderName = "rootView" + idx++;
        }
        return unbinderName;
    }
}